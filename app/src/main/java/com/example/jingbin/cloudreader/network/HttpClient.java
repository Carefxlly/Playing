...
import com.example.jingbin.cloudreader.dataclass.FrontpageBean;
import com.example.jingbin.cloudreader.dataclass.GankIoDataBean;
import com.example.jingbin.cloudreader.dataclass.GankIoDayBean;
import com.example.jingbin.cloudreader.dataclass.HotMovieBean;
import com.example.jingbin.cloudreader.dataclass.MovieDetailBean;
import com.example.jingbin.cloudreader.dataclass.UpdateBean;
import com.example.jingbin.cloudreader.dataclass.book.BookBean;
...
public interface HttpClient {
    ...
    //首页轮播图
    @GET("ting?from=android&" + "version=5.8.1.0&" + "channel=ppzs&" + "operator=3&" +
            "method=baidu.ting.plaza.index&" + "cuid=89CF1E1A06826F9AB95A34DC0F6AAA14")
    ...
    /**
     * 分类数据: http://gank.io/api/data/type/amount/page
     * type：   all Android iOS 前端 福利 拓展资源 休息视频
     */
    @GET("data/{type}/{pre_page}/{page}")
    Observable<GankIoDataBean> getGankIoData(
            @Path("type") String id,
            @Path("page") int page,
     ...
     */
    @GET("v2/movie/top250")
    Observable<HotMovieBean> getMovieTop250(
     ...
     * @param page 页码，从0开始
     */
    @GET("neihan/stream/mix/v1/?mpic=2&essence=1&content_type=-102" +
            "&message_cursor=-1&bd_Stringitude=113.369569&bd_latitude=23.149678" +
            "&bd_city=广州市&am_Stringitude=113.367846&am_latitude=23.149878&am_city=广州市" +
            "&am_loc_time=1465213692154&count=30&min_time=1465213700&screen_width=720" +
            "&iid=4512422578&device_id=17215021497&ac=wifi&channel=NHSQH5AN&aid=7" +
            "&app_name=joke_essay&version_code=431&device_platform=android&ssmix=a" +
            "&device_type=6s+Plus&os_api=19&os_version=4.4.2&uuid=864394108025091" +
            "&openudid=80FA5B208E050000&manifest_version_code=431")
     ...
     *
     * @param id       文章id
     * @param originId 列表页下发，无则为-1(代表的是你收藏之前的那篇文章本身的id,但是收藏支持主动添加，这种情况下，没有originId则为-1)
     */
    @FormUrlEncoded
    @POST("lg/uncollect/{id}/json")
    Observable<HomeListBean> unCollect(
            @Path("id") int id,
